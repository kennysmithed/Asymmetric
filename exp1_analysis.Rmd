...
These change values allow us to test whether P1 and P2 differ in the extent to which they change, and whether the conditions differ in the extent to which participants change. In order to test for interactions (i.e. do P1 and P2 differ in to the same extent in both conditions?) we calculate the difference between P1 and P2 within each pair (i.e. we take the P1 change in marker use, calculated above, and subtract the P2 change in marker use). We can then test for an interaction by comparing these difference scores across conditions - if P1 and P2 change to a similar extent in both conditions, then the P1-P2 difference scores will be similar across the two conditions; however, if P1 and P2 change to different extents across conditions, these P1-P2 difference scores will be different.
...
# We only includ eannotations for the by-participant plots where the interaction tests suggests that the effect of P1-P2 differs across conditions, i.e. in interaction.
...
#66-33 - no interaction, so not testing seperately by condition.
condition.annotations.2 <- NULL           

#83-17 - no interaction, so not testing seperately by condition.
...
#66-33 - no interaction
condition.annotations.8 <- NULL       

#83-17 - no interaction
...
###differences from 0 (no lines required for these)
###There are potentially 27 different bars to annotate
##annotations for significance - Training to Recall 1
#both conditions combined
...
#66-33 - no interaction, so no annotations
baseline.annotations.4 <- NULL
baseline.annotations.5 <- NULL
baseline.annotations.6 <- NULL

#83-17 - no interaction, so no annotations
baseline.annotations.7 <- NULL
baseline.annotations.8 <- NULL
baseline.annotations.9 <- NULL
...
baseline.annotations.11 <- data.frame(BlockToBlock="Recall 1 to Interaction Block 2",
                                      Condition="Both Conditions",
                                      Participant="P2",
                                     x=1.75,y=1/6,note="n.s.")  #add note about n.s.difference from 0
...
baseline.annotations.14 <- data.frame(BlockToBlock="Recall 1 to Interaction Block 2",
                                      Condition="Condition: 66-33",
                                      Participant="P2",
                                     x=1.75,y=1.5/6,note="n.s.")  
# given that the Ps differ, this comparison (collapsing across P, is there a reduction?) is not very meaningful so we do not test it.
...
baseline.annotations.17 <- data.frame(BlockToBlock="Recall 1 to Interaction Block 2",
                                      Condition="Condition: 83-17",
                                      Participant="P2",
                                     x=2,y=0.5/6,note="n.s.")  #add note about n.s.difference from 0
# given that the Ps differ, this comparison (collapsing across P, is there a reduction?) is not very meaningful so we do not test it.
...
#66-33 - no interaction, so no annotations
baseline.annotations.22 <- NULL
baseline.annotations.23 <- NULL
baseline.annotations.24 <- NULL

#83-17- no interaction, so no annotations
baseline.annotations.25 <- NULL
baseline.annotations.26 <- NULL
baseline.annotations.27 <- NULL
...
condition.annotations.lines.2a <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      ShortCondition=NA,
                                      x=1, y=1.1, xend=1.2, yend=1.1)
condition.annotations.lines.2b <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      ShortCondition=NA,
                                      x=1.8, y=1.1, xend=2, yend=1.1)
...
##annotations for significance - Recall 1 to Recall 2
#66-33 - no difference between conditions, so not testing conditions seperately
baseline.annotations.4 <- NULL

#83-17 - no difference between conditions, so not testing conditions seperately
baseline.annotations.5 <- NULL
