...
#use short version of condition names
# exp2.change.data.for.plotting$Condition <- plyr::revalue(exp2.change.data.for.plotting$Condition,
...
Q1: for the variably-trained participants differ from 0? do the conditions differ in change from Training to Recall 1?
...
**No significant effect** for both conditions combined.
...
```{r}
ggplot(data=subset(exp2.change.data.for.plotting.combined,BlockToBlock=="Recall 1 to Interaction Block 2"), aes(x=Participant, y=Change, fill=Participant)) +
  theme_bw() + 
  facet_grid(.~Condition)+
  geom_boxplot() +
  scale_fill_manual(values=c(my.colours[2],my.colours[1],my.colours[3])) +
...
And the same analyses separated by condition.

Q3a: do the Participants differ? **Yes for both conditions**. 
...
Q3b: do the changes differ from 0?
...
Q2a: does the change from recall 1 to recall 2 differ from 0? **Marginally**. 
```{r}
wilcox.test(exp2.change.data$ChangeRecall1Recall2) 
```

Q2b: since the effect of Participant was signicant, better break this down by P. 
...
And the same questions separated by condition.
Q3a: do the Ps differ? **No significant difference for 100-66, marginal for 100-33**. 
```{r}
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-66" & Participant=="P1")$ChangeRecall1Recall2, 
            subset(exp2.change.data,Condition=="Condition: 100-66" & Participant=="P2")$ChangeRecall1Recall2) 

wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-33" & Participant=="P1")$ChangeRecall1Recall2, 
            subset(exp2.change.data,Condition=="Condition: 100-33" & Participant=="P2")$ChangeRecall1Recall2) 
```

Q3b: does the change from recall 1 to recall 2 differ from 0? **No for 100-66 condition, yes for 100-33 condition**. 
```{r}
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-66")$ChangeRecall1Recall2) 
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-33")$ChangeRecall1Recall2) 
```

Q3c: And breaking this down by P. 

**Effect is only significant for P2s in the 100-33 condition.**
```{r}
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-66" & Participant=='P1')$ChangeRecall1Recall2) 
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-66" & Participant=='P2')$ChangeRecall1Recall2)

wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-33" & Participant=='P1')$ChangeRecall1Recall2) 
wilcox.test(subset(exp2.change.data,Condition=="Condition: 100-33" & Participant=='P2')$ChangeRecall1Recall2)
```
...
## Comnbined plot showing all three block-to-block changes.
...
#100-66 
condition.annotations.8 <- NULL #no difference
...
baseline.annotations.1 <- NULL #not testing significance for P1s 
baseline.annotations.2 <- NULL #not testing significance for P1s 
...
baseline.annotations.13 <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      Condition="Both Conditions",
                                      Participant="P1",
                                     x=0.8,y=0.5/6,note="n.s.") #add note about n.s.difference from 0
...
#100-66
baseline.annotations.15 <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      Condition="Condition: 100-66",
                                      Participant="P1",
                                     x=1,y=5/6,note="n.s.") #add note about n.s.difference from 0
baseline.annotations.16 <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      Condition="Condition: 100-66",
                                      Participant="P2",
...
#100-33
baseline.annotations.17 <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      Condition="Condition: 100-33",
                                      Participant="P1",
                                     x=1,y=0.5/6,note="n.s.") 
baseline.annotations.18 <- data.frame(BlockToBlock="Recall 1 to Recall 2",
                                      Condition="Condition: 100-33",
                                      Participant="P2",
                                     x=1.8,y=2/6,note="*")  
...
# Difference in change within each pair

This might be a way of testing for interactions - i.e. P1 and P2 change from time 1 to time 2, they might differ in the amount they change, is that difference the same across conditions (if not, that would suggest an interaction effect).

The way we calculate this is very similar to the way we calculate within-pair differences, but based on the change scores per particpant, not the raw frequencies.

```{r}
#The most convenient way to calculate differences from block to block is to restructure the data to wide format.
exp2.change.in.diffs.data <- tidyr::spread(data=dplyr::select(exp2.change.data.for.plotting,-ParticipantID),Participant,Change)

exp2.change.in.diffs.data$DifferenceInChange <- exp2.change.in.diffs.data$P1 - exp2.change.in.diffs.data$P2


```

### Plots of difference in change from Training to Recall 1 

```{r}

ggplot(data=subset(exp2.change.in.diffs.data,BlockToBlock=="Training to Recall 1"), aes(x=Condition, y=DifferenceInChange, fill=Condition)) +
  theme_bw() + 
  geom_boxplot() +
  expand_limits(y=1) +
  scale_fill_manual(values=c(my.colours[2],my.colours[1],my.colours[3])) +
  scale_y_continuous(breaks=seq(-1,1,1/3),labels=c("-1","-2/3","-1/3","0","+1/3","+2/3","+1")) +
  ylab("Difference in Change") + 
  theme(axis.title.x = element_blank()) +
  theme(legend.position="none") + 
...
```{r}
wilcox.test(subset(exp2.change.in.diffs.data,Condition=="Condition: 100-66" & BlockToBlock=="Training to Recall 1")$DifferenceInChange, 
            subset(exp2.change.in.diffs.data,Condition=="Condition: 100-33" & BlockToBlock=="Training to Recall 1")$DifferenceInChange)
```


### Plots of difference in change from Recall 1 to Interaction 2

```{r}

ggplot(data=subset(exp2.change.in.diffs.data,BlockToBlock=="Recall 1 to Interaction Block 2"), aes(x=Condition, y=DifferenceInChange, fill=Condition)) +
  theme_bw() + 
  geom_boxplot() +
  expand_limits(y=1) +
  scale_fill_manual(values=c(my.colours[2],my.colours[1],my.colours[3])) +
...
```{r}
wilcox.test(subset(exp2.change.in.diffs.data,Condition=="Condition: 100-66" & BlockToBlock=="Recall 1 to Interaction Block 2")$DifferenceInChange, 
            subset(exp2.change.in.diffs.data,Condition=="Condition: 100-33" & BlockToBlock=="Recall 1 to Interaction Block 2")$DifferenceInChange)
```

### Plots of difference in change from Recall 1 to Recall 2

```{r}

ggplot(data=subset(exp2.change.in.diffs.data,BlockToBlock=="Recall 1 to Recall 2"), aes(x=Condition, y=DifferenceInChange, fill=Condition)) +
  theme_bw() + 
  geom_boxplot() +
  expand_limits(y=1) +
  scale_fill_manual(values=c(my.colours[2],my.colours[1],my.colours[3])) +
...
```{r}
wilcox.test(subset(exp2.change.in.diffs.data,Condition=="Condition: 100-66" & BlockToBlock=="Recall 1 to Recall 2")$DifferenceInChange, 
            subset(exp2.change.in.diffs.data,Condition=="Condition: 100-33" & BlockToBlock=="Recall 1 to Recall 2")$DifferenceInChange)